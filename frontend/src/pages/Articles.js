...
function Articles() {
  ...
  return (
    <div className="container mx-auto py-10">
      <h1 className="text-4xl font-poppins font-bold mb-4">Insights & Articles</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
        {articles.map((article, idx) => (
          <motion.div
            ...
            whileTap={{ scale: 0.97 }}
          >
            <h2 className="text-xl font-poppins font-semibold mb-2 text-navy text-center">{article.title}</h2>
            <p className="text-gray-900 font-roboto flex-grow text-center">{article.content?.slice(0, 120)}...</p>
            <span className="text-xs text-gray-500 mt-2">{article.date}</span>
          </motion.div>
        ))}
      </div>
    </div>
